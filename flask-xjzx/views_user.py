...
def be_follow_userinfo(user_id):
    # 根据浏览器传过来的user_id获取要获取的被关注用户
    other_user = UserInfo.query.get(user_id)
    # 进行分页处理
    # 获取当前页码的值
    # 根据session['user_id']获取当前登录用户
    if 'user_id' in session:
    ...
    page = int(request.args.get('page', '1'))
    # 通过对象的关联属性获取相关数据
    pagination = other_user.news.order_by(NewsInfo.id.desc()).paginate(page, 4, False)
    # 获取当前页的数据
    news_list = pagination.items
    # 获取总页数
    total_page = pagination.pages

    # 判断gender属性的值，并向模板传送相应的值
    if other_user.gender == True:
        gender = '男'
    else:
        gender = '女'
...
@user_blueprint.route('/follow_info', methods=["POST"])
def follow_info():
